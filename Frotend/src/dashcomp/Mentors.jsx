                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {availableSlots.map((slot) => {
                        const isBooked = bookedSessions.some(
                          session => session.mentorId === mentor.id && session.slotId === slot.id
                        );
                        return (
                          <button
                            key={slot.id}
                            ...
                            disabled={isBooked}
                          >
                            {isBooked ? 'Booked' : `${slot.day} ${slot.time}`}
                          </button>
                        );
                      })}
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-4">
              Would you like to book a mentorship session with {selectedMentor?.name} for:
            </p>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Your mentorship session has been scheduled with {selectedMentor?.name} for {selectedSlot?.day} at {selectedSlot?.time}
              </p>
              <button
                onClick={() => {
